...
class LM(data.Dataset):
    def __init__(self, cfg, split):
        ...
        # TODO
        with open(
            r'C:\Users\lithiumice\code\CDPN_ICCV2019_ZhigangLi\dataset\000000_train_fine_2.pkl'
            ,'rb') as f:
            self.raws=pickle.load(f)
        self.num=8
    ...
    def __getitem__(self, idx):
        ...
        # TODO            
        # d.keys()
        # (['rgbs', 'depth', 'raws', 'disps', 'gt_box_corners', 'K', 'rendered_pose'])
        ...
        # raws:(8, 240, 180, 64, 4)
        # disps:(8, 240, 180)
        # gt_box_corners:(8, 1, 8, 3)
        # K:(3, 3)
        # rendered_pose:(8, 3, 5)

        ph=np.zeros(1)
        return ph, ph, self.raws['raws'][idx], self.raws['gt_box_corners'][idx], ph, ph, ph, ph, ph, ph
